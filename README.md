...
A no-op migration cannot pass by keeping the "new" adapter on the old field: the new reader must access the fixed target column, the new updater must update it directly, and the target column itself must match the independent ledger after every successful replay. This postcondition also catches an adapter that masks stale target data by reading the old field. The target column and table come from the fixed sample contract, not from the candidate.
