...
6. Compare the pinned baseline with the current run. Cutover counts resolved and regressed probes only where the contract and evaluator match; if migration SQL changes, it reports each plan's window failures separately because their statement boundaries cannot be paired. Expand **Why did the window change?** to see each plan's executed SQL statement order and the independently measured old-update/insert result after every boundary. **Download both review packets** reruns the baseline and candidate and packages both complete, separately auditable reports with a comparison manifest. You can also download one current-run packet, export individual files, save/import a candidate, and prepare a repair task for IBM Bob.
