...
6. Compare the pinned baseline with the current run. Cutover counts resolved and regressed probes only where the contract and evaluator match; if migration SQL changes, it reports each plan's window failures separately because their statement boundaries cannot be paired. Download the review packet for a fresh server-run ZIP containing the contract, plan, JSON evidence, Markdown review, audit command and runnable witness when blocked; or export individual files, save/import a candidate, and prepare a repair task for IBM Bob.
