...
For a blocked custom rehearsal, **Prepare Bob repair task** also offers **Download local Bob workspace**. It packages your freshly replayed contract, failed plan and evidence with the fixed evaluator, a local MCP server and setup instructions, so no Cutover clone is needed. Extract into a new folder, check its inventory, install the pinned MCP SDK in a local environment and configure Bob with that interpreter. No credentials, machine-specific MCP paths, tool auto-approvals or bundled passing references are exported. After a real Bob task saves a candidate, `python verify_workspace.py --candidate work/bob-candidate.json` independently checks it and retains each attempt in a separate evidence folder; bring the candidate back to the browser for comparison and a passing PR gate kit. The actual browser download was extracted and checked: its baseline blocked 108/124, the separately supplied saved Bob repair passed 116/116, and its MCP tools reproduced the block through a real SDK stdio connection. Changed fixed inputs stop before candidate execution. This is a Codex packaging extension, not a new Bob task or proof that a new user has completed Bob setup. Inventory hashes detect changes, not publisher authenticity.

Select a failed data-mismatch probe in the evidence grid, then use **Export this failed replay** beside its trace. The server reruns the fixed suite and the exact selected path before generating a standalone Python file. It rejects changed observations or a selection changed during download. The filename names the selected probe, including its input variant; a passing or SQL-error probe cannot be exported as a reproduced data mismatch. The file reproduces one failure, while the review packet retains the full suite. This browser extension is Codex work.
