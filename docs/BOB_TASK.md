...
If the IDE cannot be controlled reliably, IBM's [Bob Shell v2](https://bob.ibm.com/blog/august-2026-release/) shares task history with the IDE: start the event task in Shell, then reopen that same task in the IDE for the required screenshots. A local Bob Shell 2.0.4 client was installed from IBM's SHA-256-verified package under this task's ignored `work/bob-shell/`; `bob mcp list` recognizes the `cutover` server in the reference-withheld workspace. This is only a client/configuration check: Shell authentication, a tool call from Bob itself, and IDE history sharing have **not** yet been observed. Use Shell only after its separate license is accepted and event rules permit it.
