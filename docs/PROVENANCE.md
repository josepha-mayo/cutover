...
The later presentation finalizer, demo shot list, and local neural narration renderer are pre-event Codex preparation too. The narration template deliberately rejects its pending Bob lines, and no final MP4 has been produced. Record the last pre-kickoff commit hash here when the event opens; do not attribute any pre-kickoff commits to Bob.
