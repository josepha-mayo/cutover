...
Reproduce the table with `python -m evidence.build_ablation`; its output is [`evidence/ablation.json`](../evidence/ablation.json). Each candidate runs against fresh SQLite databases with a fixed old adapter and an independent ledger of acknowledged writes. Version 0.2.3 requires the new reader to read the target without accessing the old column, the new updater to update the target directly, and the target column to preserve ledger values. This rejects both an old-adapter no-op plan and a reader that touches the target only in a no-op expression. The JSON includes plan, contract, suite and engine hashes. The selected fixtures and input strings define the scope; these numbers are not a general accuracy, incident-reduction, or time-savings estimate.
