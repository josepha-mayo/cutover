...
Reproduce the table with `python -m evidence.build_ablation`; its output is [`evidence/ablation.json`](../evidence/ablation.json). Each candidate runs against fresh SQLite databases with a fixed old adapter and an independent ledger of acknowledged writes. Version 0.2.2 also checks that the new reader/updater uses the required target column and that column preserves ledger values; this catches a no-op plan that previously passed by reusing the old adapter. The JSON includes plan, contract, suite and engine hashes. The selected fixtures and input strings define the scope; these numbers are not a general accuracy, incident-reduction, or time-savings estimate.
