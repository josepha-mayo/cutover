...
- A three-minute [demo shot list](../presentation/recording-plan.md) and [local neural narration workflow](../presentation/VOICE.md) are prepared. A single opening-line audio sample rendered at the intended slower pace; it is not the final voiceover. Isolated browser-only takes of the public Render app checked Direct rename at `24 / 92` and Late bridge at `108 / 124`. Inspected full-frame verdict and replay frames show the latest-write mismatch. These ignored pre-event drafts need final trimming for legibility and cannot serve as Bob evidence. Bob IDE window-only footage awaits the real session. The script rejects pending Bob lines, so the final audio awaits real session evidence.
