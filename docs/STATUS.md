...
- Public [GitHub repository](https://github.com/josepha-mayo/cutover) and passing [Windows/Ubuntu CI](https://github.com/josepha-mayo/cutover/actions/runs/35876423685) on Python 3.12 for the v0.2.4 code.
...
- A v0.2.4 reference-withheld Bob repair workspace is prepared as sibling `cutover-bob-session-980d12b6` from committed Git blobs at `980d12b6`. The verifier matched all 18 copied files to that commit, confirmed the passing reference plans are absent, and recorded evaluator SHA-256 `16a6e171b8fa7a35f76d71f4ffc4940bf8787c8254cc1a4116376ca6b8e540ed`. An SDK client launched this workspace's exact `.bob/mcp.json` command, discovered all three tools, and received the expected `window_write_after_2-0` witness. The workspace generator was also exercised from a separate CRLF checkout and verified. Earlier sibling workspaces are stale. Re-freeze from the final pre-kickoff commit. This is pre-event setup, not a Bob IDE session.
...
1. Check Bob's trial capacity, hackathon access, and the September evidence rules at kickoff, then capture a real event-period Bob contribution. Event approval is confirmed; a separate access invitation was not found in email searches. Do not purchase a paid subscription.
