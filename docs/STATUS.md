...
- A v0.2.3 reference-withheld Bob repair workspace is prepared as sibling `cutover-bob-session-a32866e0` from committed Git blobs at `a32866e0`. The verifier matched all 18 copied files to that commit, confirmed the passing reference plans are absent, and recorded evaluator SHA-256 `58c8851507a8ea45b4aa1a0aa80fb400036d6b465271ffe27436d8d2c26b7cdf`. Earlier `62bea5b9` and `c1395b51` workspaces are stale; the latter also fails a committed-byte audit due Windows line endings. Re-freeze from the final pre-kickoff commit. This is pre-event setup, not a Bob session.
