...
- IBM Bob IDE opened the reference-withheld `cutover-bob-session-180246e0` workspace, but the editor currently reports **Restricted Mode** pending the user's folder-trust decision. No Bob task, MCP call from Bob, candidate, task summary, or event-period evidence has been captured. The pre-event workspace remains a connectivity rehearsal only; re-freeze from the latest clean commit when the event starts.
