...
- 27 Python tests passed on Python 3.14.7 / Windows, including migration-window detection, HTTP behavior, excluding comment-only SQL from coverage, and rejecting old-column-only plans that previously produced a false pass.
...
- Public [GitHub repository](https://github.com/josepha-mayo/cutover) and passing [Windows/Ubuntu CI](https://github.com/josepha-mayo/cutover/actions/runs/35872510361) on Python 3.12.
...
- A v0.2.3 reference-withheld Bob repair workspace is prepared as sibling `cutover-bob-session-da569646` from committed Git blobs at `da569646`. The verifier matched all 18 copied files to that commit, confirmed the passing reference plans are absent, and recorded evaluator SHA-256 `58c8851507a8ea45b4aa1a0aa80fb400036d6b465271ffe27436d8d2c26b7cdf`. An SDK client launched the exact `.bob/mcp.json` command, discovered all three tools, and received the expected `window_write_after_2-0` late-bridge witness. The workspace generator was also exercised from a separate CRLF checkout and verified. Earlier `a32866e0`, `62bea5b9`, and `c1395b51` workspaces are stale. Re-freeze from the final pre-kickoff commit. This is pre-event setup, not a Bob IDE session.
