...
- A three-minute [demo shot list](../presentation/recording-plan.md) and [local neural narration workflow](../presentation/VOICE.md) are prepared. A single opening-line audio sample rendered at the intended slower pace; it is not the final voiceover. Isolated browser-only takes of the public Render app checked Direct rename at `24 / 92` and Late bridge at `108 / 124`. A 22-second cropped draft proof clip was rendered at 1920×1080/30 fps from the live Late bridge take; inspected frames show the acknowledged old write followed by the expected `18 Marina Road` versus observed `4 Broad Street` read. These ignored pre-event drafts cannot serve as Bob evidence. Bob IDE window-only footage awaits the real session. The script rejects pending Bob lines, so the final audio awaits real session evidence.
