...
The voice renderer rejects unfinished narration, overlapping speech, or speech running past the 180-second timeline, and reports the conflicting times when a segment does not fit. The video assembler rejects pending clips, missing or too-short recordings, timing mismatches, and existing output paths. It makes a 1920×1080 H.264/AAC MP4 at 30 frames per second. A four-second test using two synthetic color clips and synthetic audio confirmed the media pipeline and cut order; those test files are ignored and cannot count as demo footage.
